...
CubeMap loadEnvMap(const std::string& imagePath)
{
	...
	if (!std::filesystem::exists(imagePath)) {
		throw std::runtime_error("The image doesn't exist: " + imagePath);
	}
	...
	bool hStrip = (width == 6 * height);           // 6 faces laid out horizontally
	bool vStrip = (height == 6 * width);            // 6 faces laid out vertically
	if (!hStrip && !vStrip)
	{
		stbi_image_free(data);
		throw std::runtime_error(
			"Expecting a 6‑face strip ((6×H)×H or H×(6×H)) but got "
			+ std::to_string(width) + "×" + std::to_string(height));
	}

	const int faceRes = hStrip ? height : width; // one edge of a square face
	...
	if (hStrip)
	{
		/* Horizontal strip:  (face 0 … 5) laid out left→right */
		for (int f = 0; f < 6; ++f)
		{
			float* dstFace = faces[f];
			for (int row = 0; row < faceRes; ++row)
			{
				const float* src = data + ((row * width) + f * faceRes) * channels;
				float* dst = dstFace + row * stride;
				std::memcpy(dst, src, stride * sizeof(float));
			}
		}
	}
	else
	{
		/* Vertical strip: (face 0 … 5) laid out top→bottom */
		for (int f = 0; f < 6; ++f)
		{
			float* dstFace = faces[f];
			const float* srcFace = data + (f * faceRes * width) * channels;
			for (int row = 0; row < faceRes; ++row)
			{
				const float* src = srcFace + row * width * channels;
				float* dst = dstFace + row * stride;
				std::memcpy(dst, src, stride * sizeof(float));
			}
		}
